...
from file_utils2048 import read_file_and_to_numpy
from file_utils2048 import read_file_and_to_numpy_val
file = './config/parameter_2048_1_9.ini'
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num):
        ...
        for step in range(1,training_steps+1):
            ...
            # batch_label = []
            # # batch_label = []
            # random_index = np.random.randint(0,2319,size=batch_size)
            # for i in random_index:
            #     for da in data[i]:
            #         batch_data.append(da)
            #     for la in label[i]:
            #         batch_label.append(la)
            # batch_data = np.array(batch_data)
           
            random_index = random.randint(0,1149)
            batch_data = data[random_index]
            batch_label = label[random_index]
            batch_data = batch_data.reshape((batch_size*2048,1,num_input))
            # 现在开始训练
