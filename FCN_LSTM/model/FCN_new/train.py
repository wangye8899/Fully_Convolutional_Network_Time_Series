...
# 定义loss和优化函数
model_ = LSTM_FCN(X,weights,biases,num_hidden)
logits = model_.Bi_LSTM_()
...
data  = tf.reshape(data,[50000,time_steps,num_input,1])
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num):
        ...
        # if epoch == 3:
        #     print("降低学习率至原来的10倍")
        #     sess.run(tf.assign(learning_rate,learning_rate/10))
        for step in range(1,training_steps+1):
            ...
            batch_label = np.reshape(batch_label,[time_steps,2])
            # 训练
