...
file = './config/parameter_1024_3_5.ini'
...
# 定义输入data、label
X = tf.placeholder(tf.float32,[None,1024,num_input],name='X')
Y = tf.placeholder(tf.float32,[None,num_classes],name='Y')

# 定义loss和优化函数
model_ = model_(X,num_hidden)
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num):
        ...
        for step in range(1,training_steps+1):
            ...
            batch_data  = tf.reshape(batch_data,[batch_size,1024,num_input])
            batch_label = tf.reshape(batch_label,[batch_size*1024,2])
