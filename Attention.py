...
class _BaseAttentionMechanism(AttentionMechanism):
  ...
  Common functionality includes:
    此时query代表解码器的当前hidden值，memory代表编码器所有的隐藏值状态值
  ...
  def __init__(self,
    ...
    参数：
        query_layer:深度与memory_layer匹配
        memory:编码器rnn的输出，形状为[batch_size,max_time,...]
        probability_fn:是一个概率转换函数、类似于softmax等函数
        memory_sequence_length:不理解，不过应该不需要指定
        memory_layer:深度与query_layer匹配
        其余不太重要
    Args:
    ...
    """
    # 检查query和memory这两个参数，非空的情况下一定要是对应的两个layer，query对应decoder、memory对应encoder
    ...
    if not callable(probability_fn):
        # probability_fn对应的参数一定是可调用并且有返回值的。否则将报错
...
class LuongAttention(_BaseAttentionMechanism):
  ...
  def __init__(self,
    ...
    参数:
        num_units:Attention机制的深度
        memory：RNN的输出，形状为[batch_size,max_time,...]
        scale:False
        probability_fn:概率函数，默认用softmax
        其余参数默认即可
    Args:
  ...
  def __call__(self, query, state):
    #  call函数中，也就是当前类被实例化调用的时候，直接会调用这个函数
    # 个人理解是key和value相等，都代表着编码器的隐藏值状态，只不过key是用来与query也就是解码器中的当前隐藏值状态计算score的，
    # 或者说是计算Attention权重的，也就是经过softmax处理的概率分布
    # 那么value也是编码器的隐藏值状态，只不过这里面是基于上一步的运算结束后，在进行加权求和，得到的权重向量
